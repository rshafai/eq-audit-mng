  ...
  return ControllerExtension.extend('gc.agr.aafc.mm.eqauditmng.ext.controller.ObjectExt', {
    ...
    _searchEquipmentMaster: function (sEquipment) {
      ...
      oListBinding.requestContexts(0, 1).then(aContexts => {
        ...
        this.getView().setBusy(false);
        MessageBox.error("Could not search equipment master data. Please try again.",
          { title: "Search Error" }
        );
      });
    },
    ...
    //────────────────────────────────────────
    _showItemAddConfirmation: function (sEquipment, oEquip) {
      const sDisplayEquip = sEquipment.replace(/^0+/, ''); // strip leading zeros for display
      this._loadAddItemDialog().then(oDialog => {
        ...
        oDialog.setModel(oAddItemModel, "addItem");
        oDialog.open();
      });
    },
    _loadAddItemDialog: function () {
      if (this._oAddItemDialog) {
        return Promise.resolve(this._oAddItemDialog);
      }
      return Fragment.load({
        id: this.getView().getId(),
        name: this._fragmentPrefix + "ConfirmAddDialog",
        controller: this
      }).then(oDialog => {
    ...
    onAddItemConfirmed: function(oEvent){
      let sEquipment = this._oAddItemDialog.getModel("addItem").getProperty("/Equipment");
      let bSuppress = this._oAddItemDialog.getModel("addItem").getProperty("/suppressConfirmation");
      if (sEquipment){
        this._suppressConfirmation = bSuppress;
        this._addEquipmentToAudit(sEquipment, true); //bBarcodeScanned=true
        this._oAddItemDialog.close();
      }
    },
